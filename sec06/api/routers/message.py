...
def message_to_schema(model: MessageModel) -> message_schema.Message:
...
def get_current_id(session: Session) -> int:
    last = session.exec(
        select(MessageModel).order_by(MessageModel.id.desc()).limit(1)
    ).first()
...
async def get_messages(session: Session = Depends(get_session),
    ...
    query = select(MessageModel).where(MessageModel.id >= from_id)
    query = query.where(MessageModel.id <= to_id)
    if from_time is not None:
        query = query.where(MessageModel.update_time >= from_time)
    if important is not None:
        query = query.where(MessageModel.important == important)
    query = query.order_by(MessageModel.id)
    ...
    message_dict = {m.id: message_to_schema(m) for m in messages if m.id is not None}
    return Response(
        current_id=get_current_id(session),
        current_time=datetime.now(),
...
async def get_message(message_id: int,
                      ...
                      current_user: User = Depends(get_current_active_user)):
    """個別 message のGET"""
    m = session.get(MessageModel, message_id)
    if m is None:
